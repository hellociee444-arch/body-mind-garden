    ...
    return (
      <div className="min-h-screen flex flex-col">
        <SEO
          ...
          description="Explore as receitas do Viva Leve organizadas por categoria: café da manhã, almoço, jantar, lanches, sobremesas e mais."
        />
        <Header />
          ...
          <section className="bg-gradient-hero py-14">
            <div className="container mx-auto px-4 max-w-3xl text-center space-y-3">
              <h1 className="font-heading text-4xl md:text-5xl font-bold text-primary-foreground text-balance">
                {slug ? "Categoria não encontrada" : "Categorias de receitas"}
              </h1>
              <p className="text-lg text-primary-foreground/90">
                Escolha uma categoria abaixo para ver as receitas.
              </p>
            </div>
          </section>
            ...
            <div className="container mx-auto px-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {CATEGORIES.map((c) => (
                <Link
                  key={c.slug}
                  to={`/categoria/${c.slug}`}
                  className="rounded-xl border border-border bg-card p-5 transition-all hover:shadow-hover hover:-translate-y-0.5"
                >
                  <h2 className="font-heading font-semibold">{c.label}</h2>
